...
public class FlightReservationDAO {
	private static final double SYSTEM_FEE = 1.1;
	...
	public List<Flight> searchFlight(FlightSearchForm flightSearchForm) {
		...
		try {
			...
			query.append(" and L1.").append(Constants.ARRIVAL_TIME).append(">=\"%s\" and ")
					.append(hasFlightFrom ^ hasFlightTo ? "L1." : "L2.").append(Constants.DEPATURE_TIME)
...
//	 		System.out.println();
	...
	public boolean reserveFlight(Customer customer, String repSSN, Flight flight) {
		...
		try {
			...
			*/
			...
			query.setLength(0);
			query.append("UPDATE ").append(Constants.CUSTOMER_TABLE)
					.append(" SET ").append(Constants.RATING_FIELD).append("=").append(Constants.RATING_FIELD)
					.append("+1 WHERE ").append(Constants.ACCOUNTNO_FIELD).append("=?;");
			stmt = conn.prepareStatement(query.toString());
			stmt.setInt(1, customer.getAccNum());
			stmt.executeUpdate();
    ...
    public boolean cancelFlight(int resrNo) {
        ...
        Connection conn = MySQLConnection.connect();
        try {
			//	UPDATE customer C JOIN reservationpassenger R ON C.AccountNo=R.AccountNo SET Rating=Rating+1 WHERE C.AccountNo=7;
			StringBuilder query = new StringBuilder("UPDATE ").append(Constants.CUSTOMER_TABLE)
					.append(" C JOIN ").append(Constants.RESERVATION_PASSENGER_TABLE).append(" R ON C.")
					.append(Constants.ACCOUNTNO_FIELD).append("=R.").append(Constants.ACCOUNTNO_FIELD)
					.append(" SET ").append(Constants.RATING_FIELD).append("=").append(Constants.RATING_FIELD)
					.append("-1 WHERE C.").append(Constants.ACCOUNTNO_FIELD).append("=?;");
			PreparedStatement stmt = conn.prepareStatement(query.toString());
			...
			query.setLength(0);
            query.append("DELETE FROM ").append(Constants.RESERVATION_TABLE)
